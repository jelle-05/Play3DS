...
> - **4.3 — quick update (echte opslag) (afgerond):** de Quick update op het dashboard slaat nu echt op via server-action `addPlaytime(playthroughId, minutes)` — telt speeltijd op, herberekent estimated progress, zet een `want_to_play`-run op `playing` (+ `started_at`), schrijft een timeline-event en `router.refresh()` ververst de kaarten. Demo-badge weg; save-knop heeft pending-staat.
> - **4.4 — playthrough-detail + timeline (afgerond):** `/playthroughs/[id]` (`force-dynamic`) toont een playthrough met cover, status, doel, speeltijd en voortgangsbalk. Owner-only `PlaythroughManager` (client): status wijzigen, speeltijd toevoegen, handmatige voortgang (slider, `progress_source='manual'`), notitie toevoegen, en verwijderen — elk via een server-action met timeline-event. De **timeline** (`getPlaythroughUpdates`) toont alle events (datum · +tijd · now · statuswissel · ±% · notitie). Dashboard-kaarten en de detail-panel linken nu naar deze pagina. **Daarmee is Fase 4 compleet.**
...
> - **5.4 — comments + `/reviews/[id]` (afgerond):** reviewdetailpagina (`force-dynamic`) met de review + comments. Server-queries `getReviewById` + `getCommentsForReview`; actions `addComment`/`deleteComment` (verwijderen via RLS owner+admin). `ReviewComments` (client) toont de lijst + plaats-formulier + delete-knoppen. Comment-telling (en like-telling) worden bij het lezen berekend via `reviewMetaFor`. De 💬-indicator op `ReviewCard` linkt naar `/reviews/[id]`. **Daarmee is Fase 5 compleet.**
